...
def firing_num(args):
    ...
    for n_iter, (input, labels) in enumerate(args.test_loader):
        ...
        if len(labels.shape) == 3:
            ...
            labels = labels.reshape(b * args.clip, labels.size()[2], labels.size()[3])
        ...
        for i in range(len(firing_nums)):
            if i < 3:
                ...
                f.create_dataset(
                    str(all_idx) + "_S_" + str(i),
                    data=firing_num_mat.squeeze().cpu().numpy(),
                )
    ...
    for n_iter, (input, labels) in enumerate(args.test_loader):
        ...
        for i in range(len(mps)):
            if i < layer:
                ...
                f.create_dataset(
                    str(all_idx) + "_S_" + str(i), data=mp.squeeze().cpu().numpy()
                )
        ...
        for i in range(b):
            ...
            if test_clip_correct / args.clip > 0.5:
                args.test_correct += 1
        args.test_acc = 100.0 * float(args.test_correct) / float(n_iter + 1)
        print(
            "iteration: {}/{}\tacc: {}".format(
                n_iter + 1, len(args.test_dataset), args.test_acc
            )
        )
...
def mean_std_firing(args):
    ...
    for t in range(T):
        for c in range(C):
            ...
            plt.savefig(
                os.path.join(
                    save_path_mean_name, "C_" + str(c) + "T_" + str(t) + ".png"
                ),
                bbox_inches="tight",
                pad_inches=0,
            )
